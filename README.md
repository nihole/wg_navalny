...
Данный волновой граф содержит логические цепочки с аргументавми за и против.
...
*Замечание 2: Это репозиторий только с данными. Скрипты и подробное пояснение, как ими пользоваться, и как строится волновой граф находятся в репозитории https://github.com/nihole/wikigraph*.
...
Этот граф представляет позицию власти по поводу отравления Навального (соответсвующий YAML файл - [kremlin.yml](https://github.com/nihole/wg_navalny/blob/main/yaml/kremlin/kremlin_navalny.yml)). Фактически, это запротоколированная логика Кремля со ссылками на соответствующие статьи. Здесь нет критики.

Вы можете скопировать svg файл к себе и открыть в любом браузере или кликнуть на приведенный ниже линк (картинка клибельна) и открыть его на github. Если вы открыли его на github обязательно выберете 'raw' в меню над картинкой. После этого тексты нод становятся кликабельными, что позволит вам легко попадать в статьи, соответсвующие вершинам графа. Также уменьшите масштаб картинки.
![kremlin.svg](https://github.com/nihole/wg_navalny/blob/main/graphs/kremlin/kremlin_navalny.svg). 


- **nw_1.svg**

Это волновой анализ только одного из утверждений власти: 'Кому он нужен?' (Путин) (соответствующий YAML файл - [nw_1.yml](https://github.com/nihole/wg_navalny/blob/main/yaml/nw_1/nw_1.yml)). Так же как и в предыдущем случае, нажмите 'Raw' и уменьшите масштаб.

!['nw_1.svg'](https://github.com/nihole/wg_navalny/blob/main/graphs/nw_1/nw_1.svg). 

Всю логику рассуждений и аргументацию вы найдете в самих волновых графах, поэтому я только хочу поделиться теми выводами, которые мы можем сделать. 

# Опорные точки

Давайте рассмотрим волновой граф, анализирующий высказываение президента "Да кому он нужен?". Это один из двух графов описанных выше, и вы можете найти его в дирректории https://github.com/nihole/wg_navalny/tree/main/graphs/nw_1 файл **nw_1.svg**. Откройте его в любом браузере (см. инструкцию выше).
